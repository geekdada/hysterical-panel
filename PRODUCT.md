...
- **Theme.** Light and dark, defaulting to system (following the OS `prefers-color-scheme`), with a manual Light / Dark / System override in the user menu persisted to `localStorage` (key `hp:theme`). No-flash on SSR via an inline `<head>` script in `frontend/src/routes/__root.tsx` that reads the saved preference (falling back to `prefers-color-scheme`) and sets the theme class before first paint, plus a live listener that keeps **System** in sync with mid-session OS switches.
