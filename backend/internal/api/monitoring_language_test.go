...
func TestMonitoringLanguageMigrationBackfillsEnglishAndRollsBack(t *testing.T) {
	...
	// The remark, online-device projection and Auth String history migrations follow the
	// notification-language migration, so roll all four back first.
	if reverted, err := runner.Down(4); err != nil || len(reverted) != 4 {
